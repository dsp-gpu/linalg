...
//
// ЧТО:    Roundtrip vs GpuKernel для матриц 341×341 и 85×85.
//         Batches: 1/2/4/8/16/32/64/128. Warmup=3, Measurement=20.
//         Замер через hipEvent (аппаратный таймер GPU).
// ЗАЧЕМ:  Симметризация — этап в Cholesky pipeline. Изолированный замер
//         выявляет, когда Roundtrip быстрее GpuKernel (малые матрицы).
// ПОЧЕМУ: Вход через void* (данные уже на GPU) — чистое GPU время.
//

...
//
// ЧТО:    Прямые вызовы rocsolver API (без класса) для точного замера:
//         Alloc → D2D copy → POTRF → POTRI → Sync → Symmetrize.
//         Матрица 341×341.
// ЗАЧЕМ:  Показывает где именно тратится время в Cholesky pipeline.
//         Нужен для оптимизации — без него нельзя обоснованно менять pipeline.
// ПОЧЕМУ: ENABLE_ROCM. Прямые rocsolver вызовы — без overhead класса.
//

...
//
// ЧТО:    1. Базовый рельеф (шум → z>0), 2. Подавление помехи (MVDR минимум
//         на θ_int=0°), 3. Адаптивное ДО, 4. Регуляризация (mu=0 vs mu>0),
//         5. GPU-to-GPU pipeline (hipMalloc + void* API).
// ЗАЧЕМ:  Capon MVDR — ключевой беамформер. Ошибка регуляризации = вырождение.
// ПОЧЕМУ: ENABLE_ROCM. Тест 2: z[m_int] < mean(z)/2 — физическое свойство MVDR.
//

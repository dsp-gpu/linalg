# Linalg — Full (объединённый: Vector Algebra + Capon)
...
**Каталог**: `linalg/`
**Зависимости**: core (`IBackend*`), rocBLAS, rocSOLVER, hiprtc, KernelCacheService
...
**Файл**: `linalg/kernels/symmetrize_upper_to_full.cl`
...
#include <dsp/linalg/vector_algebra_types.hpp>  // CholeskyResult, SymmetrizeMode
...
### ── Constructor ────────────────────────────────────────────────────────────────
inv = dsp_linalg.CholeskyInverterROCm(ctx)                              # GpuKernel
...
│                                                      │
│  ┌────────────────────┐    ┌──────────────────────┐  │
│  │ CholeskyInverterROCm│───►│ core IBackend*     │  │
│  │                    │    │ (ROCmBackend)         │  │
│  └────────────────────┘    └──────────┬───────────┘  │
│          │                            │               │
│          │                  ┌─────────▼────────────┐  │
...
**Namespace**: `capon` | **Каталог**: `linalg/`
**Зависимости**: core, **vector_algebra** (CholeskyInverterROCm), rocBLAS, rocSOLVER, hiprtc
...
| *DiagonalLoadRegularizer* | R += μI | `dsp::linalg::DiagonalLoadRegularizer` (Strategy) |
| `CaponInvertOp` | R⁻¹ | `dsp::linalg::CholeskyInverterROCm` (unique_ptr) |
...
**Прототип**: `Doc_Addition/Capon/capon_test/` — ArrayFire реализация с 6 методами инверсии. Тестировалась на P=85 каналах, 25 помехах, f0=3.9 ГГц. DSP-GPU использует typeCalc=4 (chol+inv) через `CholeskyInverterROCm`.
...
Ключевая зависимость — `dsp::linalg::CholeskyInverterROCm` (POTRF+POTRI+symmetrize). **Capon не реализует инверсию сам** — делегирует полностью. Это исключает дублирование кода и использует уже оптимизированный и протестированный модуль.
...
linalg/
  ...
  │     └── CholeskyInverterROCm  ← linalg/ (POTRF+POTRI+symmetrize)
...
dsp::linalg::CaponParams params{8, 128, 32, 0.01f};
